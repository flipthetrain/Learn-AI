...
try:
    ...
    embed_model = input('Embedding model [nomic-embed-text]: ').strip() or 'nomic-embed-text'
    gen_model   = input('Generation model [llama3]: ').strip() or 'llama3'
    ...
    doc_embeddings = ollama_embed(DOCUMENTS)
    ...
    for question in QUESTIONS:
        ...
        context = "\n\n".join(context_parts)
        prompt = (
            f"Use only the context below to answer the question. Be concise.\n\n"
            f"Context:\n{context}\n\nQuestion: {question}\nAnswer:"
        )
        answer = ollama_generate(prompt, gen_model)
